...
class BaselineKNN(Baseline):
    def get_groups_and_features(self, support_set, support_labels, query_set, way):
        ...
        support_groups = support_labels.view(-1).cpu().numpy()
        self.grouping_model = self.grouping_model.fit(support_features, support_groups)
        query_groups = self.grouping_model.predict(query_features)

        return support_groups, query_groups, support_features, query_features

    def calculate_accuracy(self, predicted_groups, true_labels):
        accuracy = accuracy_score(true_labels, predicted_groups)*100
        return accuracy
