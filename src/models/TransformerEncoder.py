...
class TransformerEncoder(nn.Module):
    def __init__(self,
        ...
        if GPICL:
            ...
            # Hardcode for rebuttal.
            self.positional_embedding = nn.Parameter(torch.zeros(1, 26, hidden_dim)).to(device)
    ...
    def forward_gpicl(self, features, labels, way, shot):
        ...
        demonstrations = self.demonstration_proj(demonstrations) +\
            self.positional_embedding[:, :demonstrations.shape[1], :]
...
class SetEncoderBlock(EncoderBlock):
    def __init__(
        ...
        self.self_attention_set = nn.MultiheadAttention(hidden_dim, num_heads,
